...
function appBack(){
  ...
  const from=history.state?.akFromRoute;
  if(from&&String(from).startsWith('#/')){history.back();return;}
  if(page==='topic'){const meta=TOPICS[p[1]];return backToParent(`#/group/${meta?.group||'skp'}`);}
  if(page==='group')return go('#/library');
  return go('#/home');
}
function updateAppBack(page){
  const b=$('#appBackButton');if(!b)return;
  const show=!['home','library'].includes(page);
...
async function route(){
  const p=parse(), page=p[0]||'home';updateAppBack(page);
...
function renderSummary(){$('#panel-summary').innerHTML=`<div class="ak-section-head"><div><h2>Inti materi</h2><p>Ringkasan cepat untuk mengingat hal yang paling penting.</p></div></div><div class="ak-deep-grid">${current.summarySections.map(s=>`<article class="ak-deep-card"><h3>${esc(s.title)}</h3><ul>${s.points.map(p=>`<li>${esc(p)}</li>`).join('')}</ul></article>`).join('')}</div><div class="ak-do-grid"><article class="ak-do"><h3>✓ Lakukan</h3><ul class="ak-list">${current.do.map(x=>`<li><span class="ak-list-mark">✓</span><span>${esc(x)}</span></li>`).join('')}</ul></article><article class="ak-dont"><h3>× Hindari</h3><ul class="ak-list">${current.dont.map(x=>`<li><span class="ak-list-mark">×</span><span>${esc(x)}</span></li>`).join('')}</ul></article></div>`}
...
async function init(){
  CATALOG=await getJSON('assets/data/catalog-v6-0-0.json?v='+ASSET_VERSION);
  if(!CATALOG || !Array.isArray(CATALOG.groupOrder) || !CATALOG.groups || !CATALOG.topics){
    throw Error('Katalog Portal v6.0.0 tidak lengkap. Muat ulang halaman atau bersihkan cache situs.');
  }
